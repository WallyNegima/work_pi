...
def labactivity():
	...
	while(1):
			...
			if len(today) == 0:
					...
					lab_days = lab_minutes/(60*24)
					lab_hours = (lab_minutes - lab_days*(60*24))/60
					lab_minutes = lab_minutes - lab_days*60*24  - lab_hours*60
					cursor.execute('insert into temp_values (date, hour) values (date(now()), 0)')
					tweet('本日までのウォーリーの研究活動時間は約%d 日と %d 時間 %d 分 です。' %(lab_days, lab_hours, lab_minutes))
					# database commit
					connector.commit()
		# 光度取得
			temp = analog_read()
...
def fork():
	...
	if pid > 0:
		f = open('/home/pi/work/labactivityd.pid','w')
		f.write(str(pid)+"\n")
		f.close()
		sys.exit()
